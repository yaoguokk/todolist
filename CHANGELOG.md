...
- **行内编辑标题**：点击待办事项标题可直接在原位编辑，支持 Enter 保存、Escape 取消、失焦自动保存。已完成事项的标题不可编辑。
...
- **截图识别结果可编辑**：弹窗内预览的识别字段改为可编辑输入框，用户可修改 AI 识别结果后再填入主表单
  - 标题/分配人/部门/责任人：文本输入框
  - 备注：多行文本框
  - DDL：datetime-local 选择器
  - "填入输入区"按钮改为从弹窗输入框读取当前值（而非原始识别结果）
